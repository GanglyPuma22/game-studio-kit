    ...
    def test_agent_log_timestamps_inside_window_break_attribution(self):
        with tempfile.TemporaryDirectory() as tmp:
            ...
            log.write_text(
                ...
                "2026-09-10T10:04:00 naive stamp secret-naive\n"
        ...
        self.assertEqual(result["contamination"]["agent_log"]["timestamps_in_window"], 2)
...
class ExecuteTests(unittest.TestCase):
    ...
    def test_real_snapshot_readers_run_on_this_host(self):
        ...
        self.assertIn(result["gpu"]["status"], {"ok", "unavailable"})
...
class HostPreflightTests(unittest.TestCase):
    ...
    def test_apply_invokes_script_with_whatif_and_receipt(self):
        with tempfile.TemporaryDirectory() as tmp:
            ...
            completed = subprocess.CompletedProcess([], 0, stdout=json.dumps({"kind": "overnight-host-preparation", "what_if": True}), stderr="")
            with patch("studio_tools.host.IS_WINDOWS", True), patch("studio_tools.host._powershell", return_value="C:/pwsh.exe"), \
                    patch("studio_tools.host.subprocess.run", return_value=completed) as runner:
