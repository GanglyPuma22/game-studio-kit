...
These rules apply whenever a prompt names an overnight, unattended, production or ledger-driven run, or hands you a production contract for a game project. They exist because one such run spent hundreds of millions of tokens and delivered zero playable minutes. The full procedure is `skills/studio-director/references/overnight-run.md` in the installed Game Studio Kit; its commands enforce what this text asks for. There is no `studio` on PATH: every `studio ...` command named below is run as `python <KIT>/scripts/studio.py <command>` through an absolute path to the installed kit, with whatever explicit project or output root that command takes (`--project` for most, `--output` for `host preflight`, a positional run root for `evidence launches`), as that procedure spells out.
...
- Never poll a running process with sleeps, waits or empty stdin writes. Launch through `python <KIT>/scripts/studio.py launch --project <run> ...` and read its single verdict when it returns. For more than one launch use `python <KIT>/scripts/studio.py batch --project <run> --plan <file>`: it is the blocking primitive that replaces a hand-rolled wait loop, so there is nothing left to poll. Separate `launch` calls are right in exactly two cases: when a run's arguments depend on an earlier run's verdict, which a fixed plan cannot express, and when each run needs its own `bench cleanroom` window, since one batch inside one wrapper yields one attribution window rather than one per capture. Never wrap these commands in a script that starts runs and then checks on them.
