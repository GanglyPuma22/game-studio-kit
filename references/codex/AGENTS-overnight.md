...
- Never poll a running process with sleeps, waits or empty stdin writes. Launch through `studio launch` and read its single verdict when it returns. For more than one launch use `studio batch --plan <file>`: it is the blocking primitive that replaces a hand-rolled wait loop, so there is nothing left to poll. Separate `launch` calls are right only when a run's arguments depend on an earlier run's verdict, which a fixed plan cannot express. Never wrap these commands in a script that starts runs and then checks on them.
