...
```json
{
  ...
  "credential_files": ["C:\\Studio Host\\keys\\meshy.env"]
}
```

`credential_files` is optional. Each entry is a host file of `KEY=VALUE` lines — a leading `export `, surrounding single or double quotes, blank lines and `#` comments are all accepted — holding the environment variable names declared in `credentials`. The process environment is still checked first, a listed file that does not exist is skipped, and a value read from a file is given only to the provider request that asked for it: it is never written into `os.environ` and never inherited by a child process this kit starts. A relative entry is resolved against the directory of this host config file, never against the working directory the command was run from, so the same config names the same file from anywhere; an absolute or UNC entry is used as written. A leading byte-order mark is tolerated. Keep those files outside the kit and outside any game project, with host-only permissions. Nothing in this repository writes them.

The optional `blender_mcp` block is needed only for the [interactive lifecycle](../skills/studio-blender/references/mcp.md); omit it when using the independent background route. Its working root must stay outside KIT. The JSON should contain escaped backslashes exactly as normal Windows JSON requires. `--config` or `STUDIO_CONFIG` selects it. Keys live in environment variables, or in the host files `credential_files` names; never in a project, a record or a command argument. Path discovery is an alternative, not a reason to edit global PATH automatically. `doctor` probes versions offline and `setup` prints missing actions; configured lifecycle paths do not establish version or interactive readiness.
