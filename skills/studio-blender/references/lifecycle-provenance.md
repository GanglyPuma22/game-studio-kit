...
The table above records the files as they were migrated, not as they are packaged now. Two changes have been made since, and both are covered by the packaged contract script and by portable tests: the loopback listener port became a host fact read from `blender_mcp.server.env.BLENDER_PORT` and passed to every script as an explicit `-Port` argument (listener checks, the endpoint-wide mutex name, the add-on bootstrap and the ownership receipt all use it, and an excluded or occupied port is refused before any launch), and `Ensure` now refuses to launch Blender while its own standard streams are a console or a pipe, because the launched process inherits them and a pipe is what kept `ensure` from returning while the GUI stayed open. The supervising Python entrypoint redirects both streams to files under the configured working root and waits a bounded 90 seconds.
