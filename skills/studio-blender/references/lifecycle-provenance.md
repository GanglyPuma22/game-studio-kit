...
The table above records the files as they were migrated, not as they are packaged now. Two changes have been made since, and both are covered by the packaged contract script and by portable tests: the loopback listener port became a host fact read from `blender_mcp.server.env.BLENDER_PORT` and passed to every script as an explicit `-Port` argument (listener checks, the endpoint-wide mutex name, the add-on bootstrap and the ownership receipt all use it, and an excluded or occupied port is refused before any launch), and `Ensure` now refuses to launch Blender while either of its own standard handles is a *pipe*, because the launched process inherits them and a pipe is what kept `ensure` from returning while the GUI stayed open. Only a pipe is refused, and the handle type is read through `GetStdHandle`/`GetFileType` rather than from a .NET stream's seekability property, which reads false for file-redirected output on Windows PowerShell 5.1 and would have refused the packaged entrypoint. The supervising Python entrypoint redirects both streams to files under the configured working root and waits a bounded 180 seconds, covering the 60-second bootstrap window and the initial probe's 75-second read timeout. The endpoint-wide lifecycle mutex is unchanged and stays host-wide (`Global\GameStudioKit-BlenderMCP`): one supervised session per host, whatever port it binds.
