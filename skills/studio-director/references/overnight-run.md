...
| 4 Performance cleanroom | root, idle | `bench cleanroom -- launch ...` with `attributable: true` | 3 | 45 | fails the frame budget → one attribution pass, no new scope |
| 5 Traversal | root | `launch` with the game's route probe and synthetic input | 3 per hypothesis | 60 | harness bug → headless fixture, never a native relaunch |
...
cleanroom capture before the next rung is added. A pass at a lower rung is
never cited for a higher one; every stage artifact carries a `scope` field.
...
```text
python <KIT>/scripts/studio.py bench cleanroom --project <GAME> --label <stage>-<n> \
  --agent-log <agent activity log> -- \
  python <KIT>/scripts/studio.py launch --project <GAME> --sha256 <engine> --mode native \
  --script <probe> --cutoff-utc <window end> --label <stage>-<n> --result <summary json>
```
...
`python <KIT>/scripts/studio.py evidence launches <run>`. Preserve failures.
