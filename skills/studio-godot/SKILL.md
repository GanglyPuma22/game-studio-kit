...
The `run` helper owns a bounded process (host timeout); a longer manual editor/play session can be launched by the host with its own ownership record. For a project-owned script that must run once and return one verdict (probes, native captures, unattended stages), use [`launch`](references/execution.md) with the engine's expected SHA-256; it blocks until the engine exits and never needs polling. To export, first configure a real project preset and install the matching export templates and set host `godot_export_templates` to their `export_templates` directory (see [isolated export setup](references/execution.md)), then use `godot export --preset <name> --output <relative build path>`. The studio does not invent credentials, export presets or target SDKs.
