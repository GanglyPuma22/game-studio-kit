...
Accepting a feature takes more than its dimension verdicts: its row in the run's feature manifest must carry a `human_verdict` and evidence recorded on the canonical route at the candidate's `content_digest` ([studio-playtest](../studio-playtest/SKILL.md)). A row whose `installed_by` points at a scene the canonical route never enters is a wiring gap, the feature exists and the game never reaches it, and that is the specific failure this check is here to catch. A creature claim that does not name its tier ([studio-game-design](../studio-game-design/SKILL.md)) is not a claim.
