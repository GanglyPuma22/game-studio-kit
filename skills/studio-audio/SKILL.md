...
The intent reserves a unique `recovery.path` beside itself before submission. Received bytes stay there, including after successful publication; preserve them as an original. `RECEIVED_UNARCHIVED` means bytes and their hash/size were saved but validation or publication failed: manually inspect/reconcile that recovery file and any existing output, without overwriting or resubmitting. `recovery.validated` reports only file validation, never listening/quality acceptance. Hard-link publication may be unsupported across filesystems; recovery still remains next to the intent. If disk writes or metadata persistence fail, the intent may remain `SUBMISSION_UNKNOWN` with a reserved path and incomplete or unconfirmed bytes. Disk exhaustion or hardware failure can prevent durable recovery; do not interpret a reserved path as proof of complete receipt.
