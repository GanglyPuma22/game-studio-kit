...
GLB is the supported requested runtime format. See [profile validation](../../../studio_tools/adapters/meshy.py) for optional fields.

The image profile accepts `should_remesh`, `target_polycount`, `topology` and `symmetry_mode` beside `ai_model`, `should_texture`, `enable_pbr` and `target_formats`. `target_polycount` is validated as an integer of 100–300000 and `topology` as `triangle` or `quad`, exactly as the preview and remesh profiles validate them; `symmetry_mode` is `off`, `auto` or `on`. The helper sets no polycount default: a request states the size it wants, or it gets the provider's raw dense sculpt and needs a separate paid remesh to become usable. For a game asset send `should_remesh: true` with an explicit count — roughly 40000–60000 for a hero landmark, 20000–30000 for a creature to be rigged, 5000–15000 for a prop — and `topology: "triangle"` for direct engine use or `"quad"` when Blender sculpt or retopology follows. Those counts are studio starting points checked against the scene budget after the first import, not provider guidance. Rig eligibility records humanoid_biped, textured, checked, clearly separated limbs and measured face count at or below 300,000. A generated success must be inspected before it becomes rig input. Source texturing/topology failures remain asset work.
